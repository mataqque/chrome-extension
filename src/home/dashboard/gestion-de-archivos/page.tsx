...
import { allFiles, selectMultiplyFile, selectSingleFile, updateFiles } from '../../../store/slice/file_managerSlice';
...
function ContentFiles() {
	const files = useSelector(allFiles);
	const dispatch = useDispatch();
	const selectFile = useCallback((file: IFileSelected) => {
			...
			<div className='grid grid-cols-[repeat(auto-fill,minmax(13rem,1fr))] gap-4 h-full scroll grid-rows-[repeat(3,max-content)] scroll pt-4 px-4'>
				{files &&
					...
					files.map((file: IFile, index: number) => {
						return <File file={file} key={'file-' + index} selectFile={selectFile}></File>;
					})}
			</div>
		</div>
	);
...
const File = memo(({ file, selectFile }: { file: IFileSelected; selectFile: (file: IFileSelected) => void }) => {
	return (
		<div
			className={`w-full  h-full  p-4 rounded-[1rem] select-none cursor-pointer shadow-1 bg-[#f8f9ff] [&.active]:bg-primary flex flex-col group ${file.selected === true ? 'active' : ''}`}
			onClick={() => {
				selectFile(file);
			}}
		>
