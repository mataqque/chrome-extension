...
import { obsToggleCategorySidebar } from './toggle';
...
export const ContentCategoryNote = () => {
	...
	useEffect(() => {
		const data = obsToggleCategorySidebar.subscribe(t => {
			useToggle(t.toggle);
		});
		return () => {
			data.unsubscribe();
	...
	return (
		...
		>
			<div className='w-full flex flex-col mobile:w-[calc(100vw-2rem)] overflow-hidden'>
				<div className='flex mb-2 pt-4 px-4 mobile:pl-0'>
					...
					<div className='flex ml-auto gap-2'>
						...
						<div
							className={`flex xsm:hidden w-7 h-7 cursor-pointer border border-solid border-[#b8cad9] rounded-lg flex items-center justify-center ml-auto cursor-pointer hover:bg-primary group duration-300 ${toggle}-button`}
							onClick={() => {
								useToggle(true);
							}}
						>
...
const ItemCategoryNote = ({ category }: { category: IProps }) => {
	...
	const handleSelect = async (c: any) => {
		obsToggleCategorySidebar.next({ toggle: true });
